...
Node* RbTree::kthNode(Node *x,int k) const {
    if (x==nil)
        return nil;
    int leftSize=x->left->size;
    if (k<=leftSize)
        return kthNode(x->left, k);
    else if (k==leftSize+1)
        return x;
    else
        return kthNode(x->right, k-leftSize-1);
}
...
void RbTree::fixSizeUpwards(Node *x) {
    while (x!=nil) {
        x->size=x->left->size+x->right->size+1;
        x=x->parent;
    }
...
int RbTree::kth(int k) const {
    Node* x=kthNode(root, k);
    if (x==nil)
        return -1;
    return x->key;
}

int RbTree::median() const {
    if (root==nil)
        return -1;
    return kth((root->size+1)/2);
}
